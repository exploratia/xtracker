// lib/src/util/logging/daily_files_io.dart
import 'dart:async';
import 'dart:io';
...
class DailyFiles {
  ...
  static Future<void> init() async {
    try {
      ...
      if (kDebugMode) {
        print('Failed to get app documents dir! $err');
      }
    }
  ...
  static void writeToFile(String value, {DateTime? dateTime}) {
    final msgQueueItem = _createMessageQueueItem(value, dateTime: dateTime);
  ...
  static Future<void> _writeToTodayFile(List<_MsgQueueItem> msgQueueItems) async {
    ...
    try {
      for (var i = 0; i < msgQueueItems.length; ++i) {
        final msgQueueItem = msgQueueItems[i];
