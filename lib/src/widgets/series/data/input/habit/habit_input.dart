...
class _HabitInputState extends State<HabitInput> {
  ...
  void _saveHandler() {
    if (!_isValid) {
      ...
      if (widget.habitValue != null) {
        _deleteHandler(widget.habitValue!);
      } else {
  ...
  void _deleteHandler(HabitValue habitValue) async {
    ...
    if (res == true && mounted) {
      Navigator.pop(context, InputResult<HabitValue>(habitValue, InputResultAction.delete));
    }
  }
          ...
          children: [
            ...
            if (widget.habitValue != null)
              IconButton(
                ...
                onPressed: () => _deleteHandler(widget.habitValue!),
